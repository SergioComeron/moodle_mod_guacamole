...
$strname = get_string('modulenameplural', 'mod_guacamole');
...
if (! $guacamoles = get_all_instances_in_course('guacamole', $course)) {
    notice(get_string('noguacamoles', 'guacamole'), new moodle_url('/course/view.php', array('id' => $course->id)));
}
...
$currentsection = '';
foreach ($modinfo->instances['guacamole'] as $cm) {
    $row = array();
    if ($usesections) {
