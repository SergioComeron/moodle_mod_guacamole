 ...
 *
 * All the guacamole specific functions, needed to implement the module
 ...
 * @param array $things
 * @return object
 *function guacamole_do_something_useful(array $things) {
 *    return new stdClass();
 *}
 */
 ...
 function guacamole_update_calendar(stdClass $guacamole, $cmid) {
    ...
    if ($event->id = $DB->get_field('event', 'id', array('modulename' => 'guacamole', 'instance' => $guacamole->id, 'eventtype' => $event->eventtype))) {
        if ((!empty($guacamole->timeopen)) && ($guacamole->timeopen > 0)) {
            ...
            $event->timesort = $guacamole->timeopen;
            $event->visible = instance_is_visible('guacamole', $guacamole);
        ...
        if ((!empty($guacamole->timeopen)) && ($guacamole->timeopen > 0)) {
            ...
            $event->groupid = 0;
            $event->userid = 0;
            $event->modulename = 'guacamole';
            $event->instance = $guacamole->id;
            $event->timestart = $guacamole->timeopen;
            $event->timesort = $guacamole->timeopen;
            $event->visible = instance_is_visible('guacamole', $guacamole);
            $event->timeduration = 0;
